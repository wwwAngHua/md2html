...
使用 t1yOS 云函数之前，你需要先前往：[console.t1y.net](https://console.t1y.net/) 创建一个应用，并打开该应用，随后在`云操作系统页面`中打开“`文件 App`“，并在“`文件 App`“ 中打开“`云函数（.functions）`“文件夹，在空白区域右键“`新建云函数`“，取名“`index`“，再次右键“`在代码编辑器中打开`“，再次点击右上角`绿色的运行按钮`，即可看到云函数执行结果。
